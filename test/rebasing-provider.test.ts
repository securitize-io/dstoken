...
describe('Rebasing', function () {
  ...
  describe('SecuritizeRebasingProvider', function () {
    ...
    const deployFixture = async () => {
      const { rebasingProvider, trustService } = await deployDSTokenRegulated();
      return { rebasingProvider, trustService };
    };
    ...
    it('should update the multiplier and emit event using master wallet', async function () {
      const { rebasingProvider } = await loadFixture(deployFixture);
    ...
    it('should fail when trying to update the multuplier from an unauthorized wallet', async function () {
      ...
      await expect(rebasingProvider.connect(unauthorized).setMultiplier(newMultiplier))
