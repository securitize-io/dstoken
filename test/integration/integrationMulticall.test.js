...
console.log("Integration Multilcall test");
contract(
  "Integration",
  function ([
    _,
    ...
    describe("Issuance", function () {
      ...
      it("should be able to issue and have a correct number of eu and us investors", async function () {
        let usInvestorsCount =
          await this.complianceService.getUSInvestorsCount.call();
        ...
        let euRetailInvestorsCount =
          await this.complianceService.getEURetailInvestorsCount.call(
            country.GERMANY
          );
    ...
    describe("Special operations", function () {
      it("Should create correctly issuer,platform and exchange wallets", async function () {
        const tx = await this.walletManager.addIssuerWallet(issuerWallet);
      ...
      it("Should allow pausing and un-pausing the token for an TRANSFER AGENT role", async function () {
        ...
        let tx = await this.token.pause({
          from: transferAgentWalletThatCanPause,
        });
        assert.equal(tx.logs[0].event, "Pause");
        // should revert
        await expectRevert.unspecified(
      ...
      it("Should allow upgrading the compliance manager", async function () {
        ...
        // // Now it should work
        await this.token.transfer(usInvestor2Wallet, 2, {
          from: usInvestorWallet,
        });
      });
      it("Should allow upgrading the token", async function () {
        ...
        await expectRevert.unspecified(
          this.token.transfer(chinaInvestorWallet, 2, {
            from: usInvestorWallet,
          })
