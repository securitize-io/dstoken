...
const HASH = '0x2bb80d537b1da3e38bd30361aa855686bde0eacd7162fef6a25fe97bf527a25b';

const hsmAddress = '0x2bD6C3D0d30AE1caa98404573FD76E8879de0EB7';
const hsmPrivateKey = '798269ec3fa0a85d8b14a704fe4abf2bf6461573a47af005a407aa1e54b809ce';
...
describe('Securitize Swap test', () => {
  describe('Deploy', () => {
    ...
    it('SHOULD usdc total supply be initialized', async () => {
      ...
      expect((await usdcMock.totalSupply()).toString()).to.be.equal(USDC_INITIAL_TOTAL_SUPPLY);
    });
        ...
        describe('[Buy Method] Interaction with Securitize Swap', () => {
          describe('[Buy Method] RegistryService integration', () => {
            it('SHOULD fail when investor is not in RegistryService', async () => {
              const { swap, navProviderMock, usdcMock } = await loadFixture(deployDSTokenRegulated);
              await navProviderMock.setRate(BigInt(rateToTest * 10 ** Number(await usdcMock.decimals())));
              const [, noInvestor] = await hre.ethers.getSigners();
              const maxExpectedStableCoin = await swap.calculateStableCoinAmount(AMOUNT_TO_BUY);
              await expect(
                swap.connect(noInvestor).buy(
          ...
          describe('[Buy Method] Security Buy', async () => {
            let swap: SecuritizeSwap, usdcMock: TokenERC20, dsToken: DSToken,
              registryService: RegistryService, navProviderMock: SecuritizeInternalNavProviderMock;
            let owner: HardhatEthersSigner, wallet: HardhatEthersSigner,
              investor1: HardhatEthersSigner, investor2: HardhatEthersSigner;
            ...
            beforeEach(async () => {
              ...
              [owner,wallet, investor1, investor2] = await hre.ethers.getSigners();
              await navProviderMock.setRate(BigInt(rateToTest * 10 ** Number(await usdcMock.decimals())));
              maxExpectedStableCoin = await swap.calculateStableCoinAmount(AMOUNT_TO_BUY);
            ...
            it('SHOULD fail when DSToken amount is 0', async () => {
              await expect(swap.connect(investor1).buy(
                0,
                maxExpectedStableCoin,
                await time.latestBlock())).to.revertedWith('DSToken amount must be greater than 0');
            });
            it('SHOULD fail when trying to buy and NAV rate is 0', async () => {
              ...
              await expect(swap.connect(investor1).buy(
                AMOUNT_TO_BUY.toString(),
                maxExpectedStableCoin,
                await time.latestBlock())).to.revertedWith('NAV Rate must be greater than 0');
            });
            it('SHOULD fail when investor has not enough stable coin balance', async () => {
              await usdcMock.connect(investor1).approve(swap.target, AMOUNT_TO_BUY);
              await expect(
                swap.connect(investor1).buy(
                  AMOUNT_TO_BUY,
                  maxExpectedStableCoin,
                  await time.latestBlock())).to.revertedWith('Not enough stable coin balance');
            });
            it('SHOULD fail when investor tries to buy DSToken and conversion is bigger tha expected', async () => {
              await usdcMock.connect(investor1).approve(swap.target, maxExpectedStableCoin);
              await expect(
                swap.connect(investor1).buy(
                  AMOUNT_TO_BUY * 200n,
                  maxExpectedStableCoin,
                  await time.latestBlock())).to.revertedWith('The amount of stable coins is bigger than max expected');
            });
            it('SHOULD process the buy', async () => {
              console.log(await usdcMock.balanceOf(wallet.address));
              console.log(usdcMock.target);
              await usdcMock.transfer(investor1.address, maxExpectedStableCoin);
              await usdcMock.connect(investor1).approve(swap.target, maxExpectedStableCoin);
              const securitizeUSDCBalance = await usdcMock.balanceOf(wallet.address);
              console.log('securitizeUSDCBalance', securitizeUSDCBalance);
              await swap.connect(investor1).buy(
                AMOUNT_TO_BUY,
                maxExpectedStableCoin,
                await time.latestBlock());
              expect(await usdcMock.balanceOf(wallet.address))
                .to.be.equal(securitizeUSDCBalance + maxExpectedStableCoin);
              expect(await dsToken.balanceOf(investor1.address)).to.be.equal(AMOUNT_TO_BUY);
            });
            it('SHOULD fail when NAV rate increases and stable coin amount is bigger than expected' +
                'with very high allowance',
            async () => {
              await usdcMock.connect(investor1).approve(swap.target, 99999999999999);
              await navProviderMock.setRate(BigInt(2 * rateToTest * 10 ** Number(await usdcMock.decimals())));
              await expect(
                swap.connect(investor1).buy(
                  AMOUNT_TO_BUY,
                  maxExpectedStableCoin,
            ...
            it('SHOULD fail when NAV rate increases and stable coin amount is bigger than expected with' +
                'very low allowance', async () => {
              ...
              async () => {
                ...
                await expect(
                  swap.connect(investor1).buy(
                    ...
                    maxExpectedStableCoin,
                    await time.latestBlock()))
                  .to.revertedWith('The amount of stable coins is bigger than max expected');
              });
            it('SHOULD process the buy when NAV rate decreases', async () => {
              await usdcMock.transfer(investor1.address, maxExpectedStableCoin);
              await usdcMock.connect(investor1).approve(swap.target, maxExpectedStableCoin);
              const balanceUsdcBefore = await usdcMock.balanceOf(investor1.address);
              const allowanceBefore = await usdcMock.allowance(investor1.address, swap.target);
              const newRate = BigInt(0.5 * rateToTest * 10 ** Number(await usdcMock.decimals()));
              await navProviderMock.setRate(newRate);
              const stableCoinBalanceAfterDecrease = await swap.calculateStableCoinAmount(AMOUNT_TO_BUY);
              expect(stableCoinBalanceAfterDecrease).to.be.lessThan(maxExpectedStableCoin);
              await swap.connect(investor1).buy(AMOUNT_TO_BUY, maxExpectedStableCoin, await time.latest());
              const balanceSecuritize = await usdcMock.balanceOf(wallet.address);
              expect(balanceSecuritize).to.be.equal(stableCoinBalanceAfterDecrease);

              const balanceUsdc = await usdcMock.balanceOf(investor1.address);
              expect(balanceUsdc).to.be.equal(balanceUsdcBefore - stableCoinBalanceAfterDecrease);

              const allowanceAfter = await usdcMock.allowance(investor1.address, swap.target);
              expect(allowanceAfter).to.be.equal(allowanceBefore - stableCoinBalanceAfterDecrease);
  ...
  describe('Swap', () => {
    ...
    beforeEach(async () => {
      ...
      [, investor01, investor02, fakeAddress] = await hre.ethers.getSigners();
      ...
      await registryService.addWallet(
        investor01.address,
        INVESTORS.INVESTOR_ID.INVESTOR_ID_1);
      await registryService.addWallet(investor02.address, INVESTORS.INVESTOR_ID.INVESTOR_ID_2);
    ...
    describe('Security Swap failure no permissions', () => {
      it('Security Swap failure no permissions', async () => {
        await expect(swap.executePreApprovedTransaction(
          signature.v,
          signature.r,
          signature.s,
          BLOCKCHAIN_ID_INVESTOR_01,
      ...
      it('SHOULD fail - Invalid signature', async () => {
        await expect(swap.executePreApprovedTransaction(
          ...
          fakeAddress.address,
          functionData,
          [VALUE, GAS_LIMIT])).to.be.revertedWith('Invalid signature');
      });
    });
    describe('Security Swap contract paused', () => {
      it('SHOULD fail - Contract Paused', async () => {
        ...
        await expect(swap.executePreApprovedTransaction(
          ...
          fakeAddress.address,
          functionData,
          [VALUE, GAS_LIMIT])).to.be.revertedWithCustomError(swap, 'EnforcedPause');
      });
    });
    describe('Security Swap failure data corruption', () => {
      it('SHOULD fail - PurchaseAmount has been changed', async () => {
        ...
        const dataChanged = [
          ...
          await time.latest(),
          HASH];
        const functionDataChanged = swap.interface.encodeFunctionData('swap', dataChanged);
    ...
    describe('Security Swap executeByInvestor interaction', () => {
      ...
      it('SHOULD fail - Incorrect params length', async () => {
        await expect(swap.executePreApprovedTransaction(
          ...
          fakeAddress.address,
          functionData,
          [VALUE])).to.be.revertedWith('Incorrect params length');
      });
      it('SHOULD fail - Transaction too old', async () => {
        ...
        const signature = await sign(
          ...
          hsmPrivateKey,
          GAS_LIMIT, VALUE);
        await expect(swap.connect(investor01).executePreApprovedTransaction(
          signature.v,
          signature.r,
          signature.s,
          BLOCKCHAIN_ID_INVESTOR_01,
          ...
          fakeAddress.address,
          functionData,
          [VALUE, GAS_LIMIT])).to.be.revertedWith('Transaction too old');
      });
      it('SHOULD fail - Investor insufficient balance', async () => {
        const [,,, fakeAddress] = await hre.ethers.getSigners();
        ...
        await expect(swap.connect(investor01).executePreApprovedTransaction(
          ...
          fakeAddress.address,
          functionData,
          [VALUE, GAS_LIMIT])).to.revertedWith('Not enough stable tokens balance');
      });
      it('SHOULD fail - Insufficient allowance', async () => {
        expect(await usdcMock.balanceOf(investor01.address)).to.be.equal(AMOUNT_TO_BUY);
        const blockNumber = await hre.ethers.provider.getBlockNumber();
        const investorWithoutAllowanceData = [
          BLOCKCHAIN_ID_INVESTOR_01,
          investor01,
          INVESTOR_COUNTRY,
          [],
        ...
        const functionDataWithoutAllowance = swap.interface.encodeFunctionData('swap', investorWithoutAllowanceData);
        const nonce = await swap.nonceByInvestor(BLOCKCHAIN_ID_INVESTOR_01);
        const signature = await sign(
          swapAddress,
          Number(nonce),
        ...
        await expect(swap.connect(investor01).executePreApprovedTransaction(
          ...
          functionDataWithoutAllowance,
          [VALUE, GAS_LIMIT]))
          .to.be.revertedWith('Not enough stable tokens allowance');
      });
    });
  });
});
