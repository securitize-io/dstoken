...
contract InvestorLockManager is ProxyTarget, Initializable, IDSLockManager, ServiceConsumer, InvestorLockManagerDataStore {
    ...
    {
        //Get total count
        uint256 lockCount = investorsLocksCounts[_holder];
        ...
        lockCount += 1;
        investorsLocksCounts[_holder] = lockCount;
    ...
    function removeLockRecordForHolder(string memory _holderId, uint256 _lockIndex) public onlyIssuerOrAbove returns (bool) {
        emit HolderUnlocked(
            _holderId,
            investorsLocks[_holderId][_lockIndex].value,
            investorsLocks[_holderId][_lockIndex].reason,
            investorsLocks[_holderId][_lockIndex].reasonString,
            investorsLocks[_holderId][_lockIndex].releaseTime
        );

        //Put the last lock instead of the lock to remove (this will work even with 1 lock in the list)
        uint256 lastLockNumber = investorsLocksCounts[_holderId];
        require(_lockIndex < lastLockNumber, "Index is greater than the number of locks");
        lastLockNumber -= 1;

        //Move the lock
        uint256 reasonCode = investorsLocks[_holderId][lastLockNumber].reason;
        setLockInfoImpl(
            _holderId,
            _lockIndex,
            investorsLocks[_holderId][lastLockNumber].value,
            reasonCode,
            investorsLocks[_holderId][lastLockNumber].reasonString,
            investorsLocks[_holderId][lastLockNumber].releaseTime
        );

        //delete the last _lock
        //Remove from reverse index
        delete investorsLocks[_holderId][lastLockNumber];

        //decrease the lock counter for the user
        investorsLocksCounts[_holderId] = lastLockNumber;
  ...
  */
    ...
    function lockCountForHolder(string memory _holderId) public view returns (uint256) {
        return investorsLocksCounts[_holderId];
    }
  ...
  */
        ...
        returns (uint256 reasonCode, string memory reasonString, uint256 value, uint256 autoReleaseTime)
    {
        uint256 lastLockNumber = investorsLocksCounts[_holderId];
        require(_lockIndex < lastLockNumber, "Index is greater than the number of locks");
        reasonCode = investorsLocks[_holderId][_lockIndex].reason;
        reasonString = investorsLocks[_holderId][_lockIndex].reasonString;
        value = investorsLocks[_holderId][_lockIndex].value;
        autoReleaseTime = investorsLocks[_holderId][_lockIndex].releaseTime;
    }
    ...
    function getTransferableTokensForHolder(string memory _holderId, uint64 _time) public view returns (uint256) {
        ...
        uint256 balanceOfHolder = getToken().balanceOfInvestor(_holderId);
        uint256 holderLockCount = investorsLocksCounts[_holderId];
        ...
        for (uint256 i = 0; i < holderLockCount; i++) {
            uint256 autoReleaseTime = investorsLocks[_holderId][i].releaseTime;

            if (autoReleaseTime == 0 || autoReleaseTime > _time) {
                totalLockedTokens = totalLockedTokens.add(investorsLocks[_holderId][i].value);
            }
        }
