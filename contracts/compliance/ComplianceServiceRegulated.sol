...
library ComplianceServiceLibrary {
    ...
    function preTransferCheck(address[] memory services, address _from, address _to, uint256 _value) public view returns (uint256 code, string memory reason) {
        ComplianceServiceRegulated complianceService = ComplianceServiceRegulated(services[COMPLIANCE_SERVICE]);

        if (IDSToken(services[DS_TOKEN]).isPaused()) {
            return (10, TOKEN_PAUSED);
        }

        if (IDSToken(services[DS_TOKEN]).balanceOf(_from) < _value) {
            return (15, NOT_ENOUGH_TOKENS);
        }

        if (
            keccak256(abi.encodePacked(IDSRegistryService(services[REGISTRY_SERVICE]).getInvestor(_from))) != keccak256("") &&
            keccak256(abi.encodePacked(IDSRegistryService(services[REGISTRY_SERVICE]).getInvestor(_from))) ==
            keccak256(abi.encodePacked(IDSRegistryService(services[REGISTRY_SERVICE]).getInvestor(_to)))
        ) {
            return (0, VALID);
        }

        uint256 fromInvestorBalance = balanceOfInvestor(services, _from);
        ...
        if (
            IDSWalletManager(services[WALLET_MANAGER]).getWalletType(_from) != IDSWalletManager(services[WALLET_MANAGER]).PLATFORM() &&
            IDSLockManager(services[LOCK_MANAGER]).getTransferableTokens(_from, uint64(now)) < _value
        ) {
        ...
        uint256 fromRegion = getCountryCompliance(services, _from);
        uint256 toRegion = getCountryCompliance(services, _to);
        ...
        if (fromRegion == US) {
            if (
                complianceService.getComplianceTransferableTokens(
                    _from,
                    uint64(now),
                    uint64(IDSComplianceConfigurationService(services[COMPLIANCE_CONFIGURATION_SERVICE]).getUsLockPeriod())
                ) <
                _value
            ) {
            ...
            if (fromInvestorBalance > _value && fromInvestorBalance.sub(_value) < IDSComplianceConfigurationService(services[COMPLIANCE_CONFIGURATION_SERVICE]).getMinUsTokens()) {
            ...
            if (
                ...
                complianceService.getComplianceTransferableTokens(
                    _from,
                    uint64(now),
                    uint64(IDSComplianceConfigurationService(services[COMPLIANCE_CONFIGURATION_SERVICE]).getNonUsLockPeriod())
                ) <
                _value
            ) {
                return (33, HOLD_UP);
            ...
            if (
                toRegion == US &&
                !(IDSWalletManager(services[WALLET_MANAGER]).getWalletType(_from) == IDSWalletManager(services[WALLET_MANAGER]).PLATFORM()) &&
                (IDSComplianceConfigurationService(services[COMPLIANCE_CONFIGURATION_SERVICE]).getBlockFlowbackEndTime() == 0 ||
                    IDSComplianceConfigurationService(services[COMPLIANCE_CONFIGURATION_SERVICE]).getBlockFlowbackEndTime() > now)
            ) {
                return (25, FLOWBACK);
            }
        }
        ...
        if (toRegion == EU) {
            if (
                isRetail(services, _to) &&
                complianceService.getEURetailInvestorsCount(getCountry(services, _to)) >=
                IDSComplianceConfigurationService(services[COMPLIANCE_CONFIGURATION_SERVICE]).getEuRetailLimit() &&
            ...
            if (balanceOfInvestor(services, _to).add(_value) < IDSComplianceConfigurationService(services[COMPLIANCE_CONFIGURATION_SERVICE]).getMinEuTokens()) {
                return (51, AMOUNT_OF_TOKENS_UNDER_MIN);
            }
        }

        if (fromRegion == EU) {
            if (fromInvestorBalance.sub(_value) < IDSComplianceConfigurationService(services[COMPLIANCE_CONFIGURATION_SERVICE]).getMinEuTokens() && fromInvestorBalance > _value) {
                return (51, AMOUNT_OF_TOKENS_UNDER_MIN);
            }
        }

        if (IDSComplianceConfigurationService(services[COMPLIANCE_CONFIGURATION_SERVICE]).getForceAccredited() && !isAccredited(services, _to)) {
            return (61, ONLY_ACCREDITED);
        }

        if (toRegion == US) {
            if (IDSComplianceConfigurationService(services[COMPLIANCE_CONFIGURATION_SERVICE]).getForceAccreditedUS() && !isAccredited(services, _to)) {
                return (61, ONLY_US_ACCREDITED);
            }

            uint256 usInvestorsLimit = getUsInvestorsLimit(services);
            if (usInvestorsLimit != 0 && fromInvestorBalance > _value && complianceService.getUSInvestorsCount() >= usInvestorsLimit && isNewInvestor(services, _to)) {
                return (41, ONLY_FULL_TRANSFER);
            }

            if (
                IDSComplianceConfigurationService(services[COMPLIANCE_CONFIGURATION_SERVICE]).getUsAccreditedInvestorsLimit() != 0 &&
                isAccredited(services, _to) &&
                complianceService.getUSAccreditedInvestorsCount() >=
                IDSComplianceConfigurationService(services[COMPLIANCE_CONFIGURATION_SERVICE]).getUsAccreditedInvestorsLimit() &&
                isNewInvestor(services, _to) &&
            ...
            if (balanceOfInvestor(services, _to).add(_value) < IDSComplianceConfigurationService(services[COMPLIANCE_CONFIGURATION_SERVICE]).getMinUsTokens()) {
                return (51, AMOUNT_OF_TOKENS_UNDER_MIN);
            }
        }
        ...
        if (!isAccredited(services, _to)) {
            if (
                IDSComplianceConfigurationService(services[COMPLIANCE_CONFIGURATION_SERVICE]).getNonAccreditedInvestorsLimit() != 0 &&
                complianceService.getTotalInvestorsCount().sub(complianceService.getAccreditedInvestorsCount()) >=
                IDSComplianceConfigurationService(services[COMPLIANCE_CONFIGURATION_SERVICE]).getNonAccreditedInvestorsLimit() &&
                isNewInvestor(services, _to) &&
        ...
        if (
            IDSComplianceConfigurationService(services[COMPLIANCE_CONFIGURATION_SERVICE]).getTotalInvestorsLimit() != 0 &&
        ...
        if (
            balanceOfInvestor(services, _from) == _value &&
            !isNewInvestor(services, _to) &&
        ...
        if (
            IDSWalletManager(services[WALLET_MANAGER]).getWalletType(_to) != IDSWalletManager(services[WALLET_MANAGER]).PLATFORM() &&
            balanceOfInvestor(services, _to).add(_value) < IDSComplianceConfigurationService(services[COMPLIANCE_CONFIGURATION_SERVICE]).getMinimumHoldingsPerInvestor()
        ) {
            return (51, AMOUNT_OF_TOKENS_UNDER_MIN);
        }

        if (
            IDSComplianceConfigurationService(services[COMPLIANCE_CONFIGURATION_SERVICE]).getMaximumHoldingsPerInvestor() != 0 &&
            balanceOfInvestor(services, _to).add(_value) > IDSComplianceConfigurationService(services[COMPLIANCE_CONFIGURATION_SERVICE]).getMaximumHoldingsPerInvestor()
        ) {
    ...
    function preIssuanceCheck(address[] memory services, address _to, uint256 _value) public view returns (uint256 code, string memory reason) {
        ComplianceServiceRegulated complianceService = ComplianceServiceRegulated(services[COMPLIANCE_SERVICE]);
        IDSComplianceConfigurationService complianceConfigurationService = IDSComplianceConfigurationService(services[COMPLIANCE_CONFIGURATION_SERVICE]);
        IDSWalletManager walletManager = IDSWalletManager(services[WALLET_MANAGER]);
        string memory toInvestor = IDSRegistryService(services[REGISTRY_SERVICE]).getInvestor(_to);
        string memory toCountry = IDSRegistryService(services[REGISTRY_SERVICE]).getCountry(toInvestor);
        ...
        if (isNewInvestor(services, _to)) {
            ...
            if (toRegion == US) {
                ...
                if (isRetail(services, _to) && complianceService.getEURetailInvestorsCount(getCountry(services, _to)) >= complianceConfigurationService.getEuRetailLimit()) {
                    return (40, MAX_INVESTORS_IN_CATEGORY);
                }
            }
        }
        ...
        if (
            ...
            balanceOfInvestor(services, _to).add(_value) < complianceConfigurationService.getMinimumHoldingsPerInvestor()
        ) {
            return (51, AMOUNT_OF_TOKENS_UNDER_MIN);
        }
        if (
            complianceConfigurationService.getMaximumHoldingsPerInvestor() != 0 &&
            balanceOfInvestor(services, _to).add(_value) > complianceConfigurationService.getMaximumHoldingsPerInvestor()
        ) {
            return (52, AMOUNT_OF_TOKENS_ABOVE_MAX);
        }
