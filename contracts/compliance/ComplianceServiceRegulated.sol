...
library ComplianceServiceLibrary {
    ...
    uint256 internal constant OMNIBUS_WALLET_SERVICE = 6;
    ...
    function isOmnibusFullWithdrawWithHolderOfRecord(address[] memory _services, address _from, address _to, uint8 _fromWalletType, uint256 _value) internal view returns (bool) {
        return
            _fromWalletType == IDSWalletManager(_services[WALLET_MANAGER]).OMNIBUS() &&
            IDSOmnibusWalletService(_services[OMNIBUS_WALLET_SERVICE]).getWalletAssetTrackingMode(_from) ==
            IDSOmnibusWalletService(_services[OMNIBUS_WALLET_SERVICE]).HOLDER_OF_RECORD() &&
            balanceOfInvestor(_services, _to) == _value;
    }

    function isOmnibusFirstDepositWithHolderOfRecord(address[] memory _services, address _to, uint8 _toWalletType) internal view returns (bool) {
        return
            _toWalletType == IDSWalletManager(_services[WALLET_MANAGER]).OMNIBUS() &&
            IDSOmnibusWalletService(_services[OMNIBUS_WALLET_SERVICE]).getWalletAssetTrackingMode(_to) ==
            IDSOmnibusWalletService(_services[OMNIBUS_WALLET_SERVICE]).HOLDER_OF_RECORD() &&
            balanceOfInvestor(_services, _to) == 0;
    }

    function isOmnibusTransfer(address[] memory _services, uint8 _fromWalletType, uint8 _toWalletType) internal view returns (bool) {
        _fromWalletType == IDSWalletManager(_services[WALLET_MANAGER]).OMNIBUS() || _toWalletType == IDSWalletManager(_services[WALLET_MANAGER]).OMNIBUS();
    }
    ...
    function preTransferCheck(address[] memory _services, address _from, address _to, uint256 _value) public view returns (uint256 code, string memory reason) {
        ...
        if (fromRegion == US) {
            ...
            if (
                !isOmnibusTransfer(_services, IDSWalletManager(_services[WALLET_MANAGER]).getWalletType(_from), IDSWalletManager(_services[WALLET_MANAGER]).getWalletType(_to)) &&
        ...
        if (toRegion == EU) {
            if (
                ...
                (isNewInvestor(_services, _to) ||
                    isOmnibusFullWithdrawWithHolderOfRecord(_services, _from, _to, IDSWalletManager(_services[WALLET_MANAGER]).getWalletType(_from), _value)) &&
        ...
        if (toRegion == US) {
            ...
            if (!isOmnibusTransfer(_services, IDSWalletManager(_services[WALLET_MANAGER]).getWalletType(_from), IDSWalletManager(_services[WALLET_MANAGER]).getWalletType(_to))) {
            ...
            if (
                ...
                (isNewInvestor(_services, _to) ||
                    isOmnibusFullWithdrawWithHolderOfRecord(_services, _from, _to, IDSWalletManager(_services[WALLET_MANAGER]).getWalletType(_from), _value) ||
                    isOmnibusFirstDepositWithHolderOfRecord(_services, _to, IDSWalletManager(_services[WALLET_MANAGER]).getWalletType(_to))) &&
    ...
    function preIssuanceCheck(address[] memory _services, address _to, uint256 _value, bool _isOmnibusIssuance) public view returns (uint256 code, string memory reason) {
        ComplianceServiceRegulated complianceService = ComplianceServiceRegulated(_services[COMPLIANCE_SERVICE]);
        ...
        IDSOmnibusWalletService omnibusWalletService = IDSOmnibusWalletService(_services[OMNIBUS_WALLET_SERVICE]);
        ...
        if (
            !_isOmnibusIssuance ||
            (walletManager.getWalletType(_to) == walletManager.OMNIBUS() && omnibusWalletService.getWalletAssetTrackingMode(_to) == omnibusWalletService.HOLDER_OF_RECORD()) ||
            (walletManager.getWalletType(_to) != walletManager.OMNIBUS() && omnibusWalletService.getWalletAssetTrackingMode(_to) == omnibusWalletService.BENEFICIAL())
        )
            if (isNewInvestor(_services, _to)) {
                ...
                if (toRegion == US) {
                    ...
                    if (
                        complianceConfigurationService.getUsInvestorsLimit() != 0 && complianceService.getUSInvestorsCount() >= complianceConfigurationService.getUsInvestorsLimit()
                    ) {
                        return (40, MAX_INVESTORS_IN_CATEGORY);
                    }
        ...
        if (!_isOmnibusIssuance || (walletManager.getWalletType(_to) != walletManager.OMNIBUS())) {
            if (
...
contract ComplianceServiceRegulated is ComplianceServiceWhitelisted {
    ...
    function recordOmnibusIssuance(address _omnibusWallet, address _to, uint256 _value, uint256 _issuanceTime) internal returns (bool) {
        if (_value != 0 && getToken().balanceOfInvestor(getRegistryService().getInvestor(_omnibusWallet)) == 0) {
            if (getOmnibusWalletService().getWalletAssetTrackingMode(_omnibusWallet) == getOmnibusWalletService().HOLDER_OF_RECORD()) {
                adjustTotalInvestorsCounts(_omnibusWallet, true);
            }
        }

        if (_value != 0 && getToken().balanceOfInvestor(getRegistryService().getInvestor(_to)) == 0) {
            if (getOmnibusWalletService().getWalletAssetTrackingMode(_omnibusWallet) == getOmnibusWalletService().BENEFICIAL()) {
                adjustTotalInvestorsCounts(_to, true);
            }
        }
    ...
    function preIssuanceCheck(address _to, uint256 _value, bool _isOmnibusIssuance) public view returns (uint256 code, string memory reason) {
        ...
        return ComplianceServiceLibrary.preIssuanceCheck(services, _to, _value, _isOmnibusIssuance);
    }
