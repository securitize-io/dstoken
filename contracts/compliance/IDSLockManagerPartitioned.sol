...
contract IDSLockManagerPartitioned is IDSLockManager {
  ...
  function removeLockRecordForHolder(string memory _investorId, uint _index, bytes32 _partition) /*issuerOrAbove*/ public returns (bool);
  ...
  function lockInfoForHolder(string memory _investorId, uint _index, bytes32 _partition) public view returns (uint reasonCode, string memory reasonString, uint value, uint autoReleaseTime);
