...
contract MultiSigWalletVersioned is VersionedContract{
    ...
    mapping (uint => string) countedTransactionIds;
    ...
    modifier notNull(address _address) {
        require(_address != 0,"Address should not be zero");
        _;
    }
        ...
        validRequirement(_owners.length, _required)
    {
        for (uint i = 0; i < _owners.length; i++) {
            require(!isOwner[_owners[i]] && _owners[i] != 0,"Error adding owner");
    ...
    function submitTransaction(string memory transactionId,address destination, uint value, bytes memory data)
        public
    ...
    function confirmTransaction(string memory transactionId)
        public
    ...
    /// @param transactionId Transaction ID.
    function revokeConfirmation(string memory transactionId)
        public
        ownerExists(msg.sender)
    ...
    /// @param transactionId Transaction ID.
    function executeTransaction(string memory transactionId)
        public
        ownerExists(msg.sender)
        confirmed(transactionId, msg.sender)
        notExecuted(transactionId)
    ...
    function isConfirmed(string memory transactionId)
        public
        view
        returns (bool)
    {
    ...
    /// @param data Transaction data payload.
    /// @return Returns transaction ID.
    function addTransaction(string memory transactionId,address destination, uint value, bytes memory data)
        internal
        notNull(destination)
    {
    ...
    function getConfirmationCount(string memory transactionId)
        public
        view
        returns (uint count)
    {
        ...
        returns (uint count)
    {
        ...
        for (uint i = 0; i < transactionCount; i++)
            ...
            if (   pending && !transactions[transactionId].executed
                || executed && transactions[transactionId].executed)
                count += 1;
    }
    ...
    function getConfirmations(string memory transactionId)
        public
        view
