...
const EternalStorageClientUintLibrary = artifacts.require('EternalStorageClientUintLibrary');
const EternalStorageClientAddressLibrary = artifacts.require('EternalStorageClientAddressLibrary');
const EternalStorageClientBooleanLibrary = artifacts.require('EternalStorageClientBooleanLibrary');
const EternalStorageClientStringLibrary = artifacts.require('EternalStorageClientStringLibrary');
const ESComplianceServiceLibrary = artifacts.require('ESComplianceServiceLibrary');
...
const EternalStorageClients = [ESTrustService,
                               DSToken,
...
module.exports = function (deployer) {
  deployer.then(async () => {
    ...
    // Deploy eternal storage
    await deployLibraries(deployer).then(() => {
      ...
      case 'NORMAL':
        ...
        return deployer.deploy(ESComplianceServiceLibrary).then(async s => {
          ...
          await deployer.link(ESComplianceServiceLibrary, ESComplianceServiceRegulated);
      ...
      return deployer.deploy(ESComplianceConfigurationService, storage.address, `${name}ComplianceConfiguration`);
    }).then(s => {
      ...
      console.log('Adding write right on eternal storage to compliance configuration service');
      ...
      return complianceConfiguration.setDSService(TRUST_SERVICE, trustService.address);
    }).then(() => {
      ...
      console.log('Connecting compliance manager to compliance configuration service');
      return complianceService.setDSService(COMPLIANCE_CONFIGURATION_SERVICE, complianceConfiguration.address);
    }).then(() => {
      if (registry) {
      ...
      console.log('-------------------------');
      console.log(`Token is at address (2): ${token.address} (behind proxy) Version: ${await token.getVersion()}`);
      console.log(`Trust service is at address (1): ${trustService.address} Version: ${await trustService.getVersion()}`);
      if (registry) {
        console.log(`Investor registry is at address (4): ${registry.address} Version: ${await registry.getVersion()}`);
      }
      console.log(`Compliance service is at address (8): ${complianceService.address}, and is of type ${complianceManagerType}. Version: ${await complianceService.getVersion()}`);
      console.log(`Compliance configuration service is at address (256): ${complianceConfiguration.address}. Version: ${await complianceConfiguration.getVersion()}`);
      console.log(`Wallet manager is at address (32): ${walletManager.address} Version: ${await walletManager.getVersion()}`);
      console.log(`Lock manager is at address (64): ${lockManager.address}, and is of type ${lockManagerType}. Version: ${await lockManager.getVersion()}`);
      console.log(`Eternal storage is at address: ${storage.address} Version: ${await storage.getVersion()}`);
      console.log(`Token implementation is at address: ${tokenImpl.address} Version: ${await tokenImpl.getVersion()}`);
      console.log(`Token issuer is at address: ${tokenIssuer.address} Version: ${await tokenIssuer.getVersion()}`);
      console.log(`Wallet registrar is at address: ${walletRegistrar.address} Version: ${await walletRegistrar.getVersion()}`);
